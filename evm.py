...
# Function to generate a human-readable random contract name and details
...
# Function to deploy ERC-20 contract
def deploy_contract(private_key):
    ...
    print(f"Deployed contract {contract_details['name']} ({contract_details['symbol']}) at {receipt.contractAddress}")
    return receipt.contractAddress

# Function to send 0 ETH transaction to deployed contracts
def send_0_eth_transaction(private_key, repetitions):
    sender_address = web3.eth.account.from_key(private_key).address

    for i in range(repetitions):
        to_address = deploy_contract(private_key)  # Deploy new contract
        ...
        for attempt in range(MAX_RETRIES):
            try:
                ...
                gas_limit = int(estimated_gas_limit * 1.2)  # Add 20% buffer
        ...
        time.sleep(5)  # Avoid nonce issues
...
if __name__ == "__main__":
    ...
    send_0_eth_transaction(private_key, repetitions)
